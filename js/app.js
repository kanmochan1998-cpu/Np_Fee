...
init() {
    ...
    renderItems() {
        ...
        ['person', 'vehicle', 'exemption', 'sleep'].forEach(group => {
            ...
            items.forEach(item => {
                ...
                // --- 1. ปรับลูกเล่นเงา (Shadow) และสีพื้นหลังให้สวยงามขึ้น ---
                let glow = isActive 
                    ? `ring-2 ring-opacity-50 border-transparent ${
                        group === 'person' ? 'ring-emerald-500 bg-emerald-50 dark:bg-emerald-900/60 shadow-lg shadow-emerald-500/30' :
                        group === 'vehicle' ? 'ring-blue-500 bg-blue-50 dark:bg-blue-900/60 shadow-lg shadow-blue-500/30' :
                        group === 'sleep' ? 'ring-orange-500 bg-orange-50 dark:bg-orange-900/60 shadow-lg shadow-orange-500/30' :
                        'ring-purple-500 bg-purple-50 dark:bg-purple-900/60 shadow-lg shadow-purple-500/30'
                      }` 
                    : 'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 shadow-lg shadow-slate-200/50 dark:shadow-slate-900/80';
                ...
                // --- 2. อัปเกรดคลาสของต้วการ์ด (เพิ่ม hover ให้การ์ดเด้งและเงาขยายขึ้น) ---
                card.className = `relative rounded-2xl p-3 transition-all duration-200 h-24 border ${glow} hover:-translate-y-1 hover:shadow-xl active:scale-95 cursor-pointer select-none flex flex-col justify-between`;
                ...
                card.innerHTML = `
                    ${(item.type === 'sleep' && isActive) ? 
                        `<div class="absolute -top-2 -left-2 bg-orange-500 text-white px-2 h-6 flex items-center justify-center rounded-full shadow-md font-bold text-[10px] z-10 heartbeat-active">
                    ...
                    ${isActive ? `<button onclick="event.stopPropagation(); app.updateCart('${item.id}', -1)" class="absolute -top-2 -right-2 w-7 h-7 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md font-bold hover:bg-red-600 transition z-10">-</button>` : ''}
                `;
   ...
   history: { 
        ...
        open() {
            ...
            setTimeout(() => document.getElementById('historyOverlay').classList.remove('translate-y-full'), 10);
            
            // รีเซ็ตเป็นวันปัจจุบันเสมอเมื่อเปิดหน้าต่าง
